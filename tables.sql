...
create or replace PROCEDURE hirdeto_create (
    username IN hirdetok.felhasznalonev%TYPE,
    pass IN hirdetok.jelszo%TYPE,
    email IN hirdetok.email%TYPE,
    phone IN hirdetok.telefon%TYPE,
    legal_name IN hirdetok.nev%TYPE,
    cegid IN hirdetok.cegid%TYPE)
IS
BEGIN
    INSERT INTO hirdetok (
        felhasznalonev,
        jelszo,
        email,
        telefon,
        nev,
        cegid) 
        VALUES (username, pass, email, phone, legal_name,cegid);
END;
...
BEFORE UPDATE OR INSERT ON hirdetesek
FOR EACH ROW 
DECLARE 
...
FOR EACH ROW
BEGIN
    DELETE FROM hirdetes_jellege WHERE hirdetesID = :OLD.hirdetesID;
END;
